...
class TestCalibrate(object):
    ...
    def calib(self):
        ...
        fake_counters = np.array([np.array([np.zeros(num_obs),np.zeros(num_obs)]).T, x_no_noise])
...
class TestErupt(object):
    ...
    def test_erupt(self):
        ...
        expected_means = y_no_noise[np.where((tmt_no_noise == (x_no_noise[:,0] - x_no_noise[:,1] > 0))) [0]].mean(axis=0)

        erupts = erupt(y_no_noise, tmt_no_noise, optim_tmt, weights = None, names = None).iloc[:,0]
...
class TestIbottaUplift(object):
    ...
    def test_model_pred_oos_shapes(self):
        ...
        t = np.concatenate([t.reshape(-1,1), np.random.binomial(1,.5, num_obs).reshape(-1,1)], axis = 1)

        uplift_model = IbottaUplift()
        uplift_model.fit(x,y,t, n_jobs = 1)

        assert uplift_model.predict_ice().shape == (np.unique(t, axis = 0).shape[0], num_obs*.7, y.shape[1])
        assert uplift_model.predict_ice(x = x).shape == (np.unique(t, axis = 0).shape[0], num_obs, y.shape[1])
